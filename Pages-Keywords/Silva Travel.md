# Project Repository: Silva Travel

This document serves as the master data source for the Silva Travel project. It contains all approved target pages, keywords, profile links, business listings, and the GMB link required for the off-page content creation workflow.

---

## 1. Target Pages and Keywords

| Target URL | Approved Keywords |
| :--- | :--- |
| `https://silvatravel.co.nz/` | Travel Agents Auckland, Travel Agency Auckland, Best Travel Agency In Auckland |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-united-arabs` | Flights from Auckland to United Arabs |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-gold-coast` | Flights from Auckland to Gold Coast |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-adelaide` | Flights from Auckland to Adelaide |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-abu-dhabi` | Flights from Auckland to Abu Dhabi |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-thiruvananthalam` | Flights from Auckland to Thiruvananthalam |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-australia` | Flights from Auckland to Australia |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-dubai` | Flights from Auckland to Dubai |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-hyderabad` | Flights from Auckland to Hyderabad |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-mumbai` | Flights from Auckland to Mumbai |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-colombo` | Auckland to Colombo flights |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-hambantota` | Flights from Auckland to Hambantota |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-india` | cheap flights from Auckland to India |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-fiji` | cheapest flights from Auckland to Fiji |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-chennai` | flight tickets from Auckland to Chennai |
| `https://silvatravel.co.nz/destinations/flights-from-auckland-to-delhi` | flights from Auckland to Delhi |

---

## 2. Placement Targets by Content Type

### 2.1 Web 2.0 Target Pages

| # | Target URL | Notes |
| :--- | :--- | :--- |
| 1 | `https://silvatravel.co.nz/` | Travel Agents Auckland |
| 2 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-united-arabs` | Flights from Auckland to United Arabs |
| 3 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-gold-coast` | Flights from Auckland to Gold Coast |
| 4 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-adelaide` | Flights from Auckland to Adelaide |
| 5 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-abu-dhabi` | Flights from Auckland to Abu Dhabi |
| 6 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-hyderabad` | Flights from Auckland to Hyderabad |
| 7 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-mumbai` | Flights from Auckland to Mumbai |
| 8 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-colombo` | Auckland to Colombo flights |
| 9 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-hambantota` | Flights from Auckland to Hambantota |

---

### 2.2 Guest Post Target Pages

| # | Target URL | Notes |
| :--- | :--- | :--- |
| 1 | `https://silvatravel.co.nz/` | Travel Agency Auckland |
| 2 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-thiruvananthalam` | Flights from Auckland to Thiruvananthalam |
| 3 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-adelaide` | Flights from Auckland to Adelaide |
| 4 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-india` | cheap flights from Auckland to India |
| 5 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-fiji` | cheapest flights from Auckland to Fiji |
| 6 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-colombo` | Auckland to Colombo flights |
| 7 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-hambantota` | Flights from Auckland to Hambantota |

---

### 2.3 Article Submission Target Pages

| # | Target URL | Notes |
| :--- | :--- | :--- |
| 1 | `https://silvatravel.co.nz/` | Best Travel Agency In Auckland |
| 2 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-australia` | Flights from Auckland to Australia |
| 3 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-dubai` | Flights from Auckland to Dubai |
| 4 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-chennai` | flight tickets from Auckland to Chennai |
| 5 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-delhi` | flights from Auckland to Delhi |
| 6 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-hyderabad` | Flights from Auckland to Hyderabad |
| 7 | `https://silvatravel.co.nz/destinations/flights-from-auckland-to-mumbai` | Flights from Auckland to Mumbai |

---

## 3. Approved Profile Links

| Platform | Profile URL |
| :--- | :--- |
| Express Africa | `https://expressafrica.net/silvatravels` |
| The Anti Racist Educator | `https://www.theantiracisteducator.com/profile/silvatravel66485613/profile` |
| Buyer Seller | `https://buyerseller.xyz/user/silvatravels/` |
| MyBB Community | `https://community.mybb.com/user-161523.html` |
| Sport and Dev | `https://www.sportanddev.org/user/silva-travel-and-tours` |
| Boycat | `https://www.boycat.co/silvatravels` |
| Zintro | `https://app.zintro.com/profile/ziae82fec5` |
| Linus Tech Tips | `https://linustechtips.com/profile/1217937-silva-travel-and-tours/` |
| Speedrun | `https://www.speedrun.com/users/silvatravel664` |
| Social Finder | `https://socialfinder.app/@silvatravels` |
| Solo.to | `https://solo.to/silvatravelandtours` |
| Bio.site | `https://bio.site/silvatravels664` |
| Home Assistant | `https://community.home-assistant.io/t/silva-travel-and-tours/1013446?u=silvatravel664` |
| Site Price | `https://www.siteprice.org/website-worth/silvatravel.co.nz` |
| Tinytail | `https://www.tinytail.io/p/silvatravels` |
| OneURL | `https://oneurl.live/silvatravels` |
| Biolink | `https://biolink.page/silvatravels` |
| Inlink | `https://inlink.bio/silvatravels` |
| Lnk.Bio | `https://lnk.bio/silvatravels` |
| Many.bio | `https://many.bio/silvatravels` |
| Lin.ky | `https://lin.ky/e/silvatravels` |
| Rootslink | `https://www.rootsl.ink/silvatravels` |
| Dropbio | `https://dropbio.bio/silvatravels` |
| Linklay | `https://linklay.io/silvatravel` |
| Linkai | `https://linkai.ink/silvatravels` |

---

## 4. Business Directory and Citation Links

| Platform | Listing URL |
| :--- | :--- |
| Biz To Biz | `https://www.biztobiz.org/travel-recreation-and-leisure/silva-travel-and-tours` |
| Self Employed AI | `https://www.selfemployedai.com/business-services/silva-travel-and-tours` |
| Blog Bang Boom | `https://www.blogbangboom.com/travel-recreation-and-leisure/silva-travel-and-tours` |
| America Small Biz | `https://www.americasmallbiz.com/travel-recreation-and-leisure/silva-travel-and-tours` |
| Big Biz Stuff | `https://www.bigbizstuff.com/travel-recreation-and-leisure/silva-travel-and-tours` |
| Biz 411 | `https://www.biz411.org/travel-recreation-and-leisure/silva-travel-and-tours` |
| Biz Bang Boom | `https://www.bizbangboom.com/travel-recreation-and-leisure/silva-travel-and-tours` |
| Biz Build Boom | `https://www.bizbuildboom.com/travel-recreation-and-least/silva-travel-and-tours` |
| Bizforge USA | `https://bizforgeusa.com/travel-recreation-and-tours` |
| Sayellow | `https://www.sayellow.com/view/new-zealand/silva-travels-and-tours-ltd-in-auckland` |
| Muamat | `https://www.muamat.com/classifieds/759/posts/1_Services/13_Other/45876307_Silva_Travels_and_Tours_Ltd.html` |
| Yellow NZ | `https://yellow.co.nz/profile/1320283333/reviews` |
| Brownbook | `https://www.brownbook.net/business/53988708/silva-travels-tours-ltd` |
| Get Listed UAE | `https://www.getlisteduae.com/listings/silva-travels-and-toursltd-1` |
| Australian Dir | `https://www.australiandir.com/website-info/654deea5e6d1aa2789b60805c6723bf9/` |
| Sitelike | `https://www.sitelike.org/similar/silvatravel.co.nz/` |
| Bizpages | `https://bizpages.org/business--New-Zealand--689-Sandringham-Road-Sandringham-Auckland-1041-New-Zealand--54682` |
| Trepup | `https://www.trepup.com/@silvatravels` |
| Globe Connected | `https://globeconnected.com/Silva-Travels-and-Tours%C2%A0Ltd` |
| Omiyou | `https://omiyou.com/silvatravels` |
| Free Listing Australia | `https://www.freelistingaustralia.com/listings/silva-travels-and-toursltd` |
| Directory9 | `https://directory9.net/listing/silva-travels-and-toursltd-68708ee7e9619.html` |
| GPS Listings | `https://gpslistings.com/listing/silva-travels-and-tours-ltd-1/` |
| Choice Bookmarks | `https://www.choicebookmarks.com/listing/silva-travels-and-tours-ltd-2/` |
| Globale Connections | `https://www.globaleconnections.com/new-zealand/auckland/hotels-travel/silva-travels-and-tours%C2%A0ltd` |
| NZ SB Directory | `https://www.nzsbdirectory.co.nz/listing/silva-travels-and-toursltd.html` |
| Business Networking NZ | `https://www.businessnetworking.nz/listing/857196` |
| Business Listings AU | `https://www.businesslistings.net.au/travel_agency/Auckland/Silva_Travels_and_Tours%C2%A0Ltd/1148585.aspx` |

---

## 5. GMB (Google My Business) Link

| Property | URL |
| :--- | :--- |
| Google Maps / GMB | `https://www.google.com/maps?cid=3494269107193263420` |

---

## 6. Project Homepage

| Property | URL |
| :--- | :--- |
| Website | `https://silvatravel.co.nz/` |
